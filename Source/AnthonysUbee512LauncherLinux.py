...
APP_NAME = "Anthony's Ubee512 Launcher"
...
class UbeeLauncherApp:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(top, text="Auto", command=self.find_ubee_in_path, width=8).grid(row=0, column=5, padx=(6, 0), pady=2)
        ttk.Button(top, text="Find", command=self.choose_ubee_executable, width=10).grid(row=0, column=6, padx=(6, 0), pady=2)
    ...
    def find_ubee_in_path(self) -> None:
        """Find ubee512 automatically using PATH first, then a user-selected folder."""
        found = shutil.which("ubee512")
        if found:
            self.ubee_var.set(found)
            self.status_var.set(f"Found ubee512 in PATH: {found}")
            self.refresh_command_preview()
            return

        folder = filedialog.askdirectory(
            title="ubee512 was not found in PATH. Select your uBee512 folder to search inside."
        )
        if not folder:
            self.status_var.set("Auto find cancelled. ubee512 was not found in PATH.")
            return
        ...
        try:
            for candidate in root.rglob("ubee512"):
                if candidate.is_file() and os.access(candidate, os.X_OK):
                    matches.append(candidate)
        except PermissionError:
            pass
        except OSError as exc:
            messagebox.showerror(APP_NAME, f"Could not search that folder:\n\n{exc}")
            self.status_var.set("Auto find could not search the selected folder.")
            return

        if matches:
            matches.sort(key=lambda p: ("/bin/" not in str(p), len(str(p)), str(p).lower()))
            selected = matches[0]
            self.ubee_var.set(str(selected))
            if len(matches) == 1:
                self.status_var.set(f"Found ubee512: {selected}")
            else:
                self.status_var.set(f"Found {len(matches)} ubee512 files. Using: {selected}")
            self.refresh_command_preview()
            return

        messagebox.showerror(
            APP_NAME,
            "Could not find an executable file named ubee512 in that folder. "
            "Use Find to select it manually.",
        )
        self.status_var.set("ubee512 was not found in PATH or the selected folder.")
