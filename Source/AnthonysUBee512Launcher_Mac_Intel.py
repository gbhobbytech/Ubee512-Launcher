...
A macOS desktop launcher for Ubee512 that:
...
CONFIG_DIR = Path.home() / "Library" / "Application Support" / "Ubee512 Launcher"
CONFIG_FILE = CONFIG_DIR / "config.json"
DEFAULT_WINDOW_GEOMETRY = "1180x740"

DEFAULT_UBEE_EXECUTABLE = Path("ubee512")
...
DEFAULT_CONFIG = {
   ...
   "emulator_title": "Launched with Anthony's Ubee512 Launcher",
    ...
    "mounted_drive_d": "",
    "advanced_mode": False,
    "show_printer_launch_reminder": True,
    "show_tape_launch_reminder": True,
...
class ToolTip:
    """Small hover tooltip with macOS-compatible behaviour."""
...
class UbeeLauncherApp:
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.show_printer_launch_reminder_var = tk.BooleanVar(
            value=self.config.get("show_printer_launch_reminder", DEFAULT_CONFIG["show_printer_launch_reminder"])
        )
        self.show_tape_launch_reminder_var = tk.BooleanVar(
            value=self.config.get("show_tape_launch_reminder", DEFAULT_CONFIG["show_tape_launch_reminder"])
        )
    ...
    def save_config(self) -> None:
        ...
        payload = {
            ...
            "show_printer_launch_reminder": self.show_printer_launch_reminder_var.get(),
            "show_tape_launch_reminder": self.show_tape_launch_reminder_var.get(),
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(
            mounted_panel,
            text="Select tape  >  Use tape or double-click  >  Launch",
            justify=tk.LEFT,
    ...
    def _bind_events(self) -> None:
        ...
        self.disk_list.bind("<Double-Button-1>", self.on_disk_double_click)
        self.tape_list.bind("<Double-Button-1>", self.on_tape_double_click)
    ...
    def find_ubee_executable(self) -> str:
        """Locate uBee512 on macOS without hard-coding one user's install."""
        found = shutil.which("ubee512")
        if found:
            return found

        search_patterns = [
            "/Applications/ubee512-*/ubee512/app/ubee512",
            "/Applications/ubee512*/ubee512/app/ubee512",
            str(Path.home() / "Applications" / "ubee512-*" / "ubee512" / "app" / "ubee512"),
            str(Path.home() / "Downloads" / "ubee512-*" / "ubee512" / "app" / "ubee512"),
        ]

        matches: list[Path] = []
        for pattern in search_patterns:
            matches.extend(Path(match).expanduser() for match in glob.glob(pattern))

        executable_matches = [path for path in matches if path.is_file() and os.access(path, os.X_OK)]
        if executable_matches:
            executable_matches.sort(key=lambda path: str(path).lower())
            return str(executable_matches[-1])

        file_matches = [path for path in matches if path.is_file()]
        if file_matches:
            file_matches.sort(key=lambda path: str(path).lower())
            return str(file_matches[-1])

        return ""

    def auto_setup(self) -> None:
        found = self.find_ubee_executable()
        if found:
        ...
        if found:
            ...
            self.status_var.set("Auto setup scanned ~/.ubee512. Choose the uBee512 executable manually.")
            messagebox.showinfo(
                APP_NAME,
                "Auto setup scanned ~/.ubee512, but could not find the uBee512 executable.\n\nUse the Find button to select it manually.",
            )
    ...
    def clear_config(self) -> None:
        ...
        self.show_printer_launch_reminder_var.set(DEFAULT_CONFIG["show_printer_launch_reminder"])
        self.show_tape_launch_reminder_var.set(DEFAULT_CONFIG["show_tape_launch_reminder"])
    ...
    def _build_diagnostics_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Button(button_row, text="Reset launch reminders", command=self.reset_launch_reminders).pack(side=tk.LEFT, padx=(8, 0))
    ...
    def refresh_diagnostics(self) -> None:
        ...
        library_path = self.library_path_var.get().strip()
        if library_path:
            lines.append(self.info_line("Library path override", f"{library_path} (added to DYLD_LIBRARY_PATH when launching)"))
        else:
            lines.append(self.info_line("Library path override", "not set; most uBee512 installations should leave this blank"))
        lines.append(self.info_line("Printer launch reminder", "enabled" if self.show_printer_launch_reminder_var.get() else "disabled"))
        lines.append(self.info_line("Tape launch reminder", "enabled" if self.show_tape_launch_reminder_var.get() else "disabled"))
        lines.append("")
        ...
        if ubee_found:
            ...
            if ubee_path and ubee_path.exists():
                try:
                    result = subprocess.run(["otool", "-L", str(ubee_path)], capture_output=True, text=True, check=False, timeout=3)
                    output = (result.stdout or result.stderr).strip()
                    if result.returncode == 0 and output:
                        lines.append(self.diagnostics_line("Shared libraries", True, "otool -L completed"))
                    else:
                        lines.append(self.info_line("Shared libraries", "otool -L produced no library report"))
                except Exception:
                    pass
    ...
    def scan_folder_for_extensions(self, folder: Path, extensions: set[str]) -> list[Path]:
        """Return files below folder that match the supplied extension set."""
        matches: list[Path] = []
    ...
    def find_files(self, root_path: Path) -> ScanResults:
        """Scan for uBee512 media files.

        A normal uBee512 setup keeps media in roms/, disks/, and tapes/
        below the selected root. When those folders are present, scan each
        expected folder separately so sibling folders such as xtapes/ or old/
        are not mixed into the launcher lists.

        If the selected root is a custom folder that does not contain any of
        the expected uBee512 media folders, fall back to the previous broad
        recursive scan so users can still point the launcher at an arbitrary
        collection folder.
        """
        root_path = root_path.expanduser()
        roms_dir = root_path / "roms"
        disks_dir = root_path / "disks"
        tapes_dir = root_path / "tapes"
        has_expected_media_folders = any(
            folder.exists() and folder.is_dir()
            for folder in (roms_dir, disks_dir, tapes_dir)
        )

        if has_expected_media_folders:
            roms = self.scan_folder_for_extensions(roms_dir, ROM_EXTENSIONS)
            disks = self.scan_folder_for_extensions(disks_dir, DISK_EXTENSIONS)
            tapes = self.scan_folder_for_extensions(tapes_dir, TAPE_EXTENSIONS)
        else:
            roms = []
            disks = []
            tapes = []
            for dirpath, _dirnames, filenames in os.walk(root_path):
                for name in filenames:
                    path = Path(dirpath) / name
                    if has_extension(path, ROM_EXTENSIONS):
                        roms.append(path)
                    elif has_extension(path, DISK_EXTENSIONS):
                        disks.append(path)
                    elif has_extension(path, TAPE_EXTENSIONS):
                        tapes.append(path)
    ...
    def on_tape_selection_changed(self, _event=None) -> None:
        ...
        if tape is not None:
            self.status_var.set(f"Selected tape: {tape.name}. Press 'Use tape' or double-click to mount it as input.")

    def on_tape_double_click(self, _event=None) -> None:
        self.use_selected_tape_input()

    def on_disk_double_click(self, _event=None) -> None:
        disk = self.get_selected_disk_path()
        if disk is None:
            return
        if self.is_ide_image(disk):
            self.status_var.set("IDE/CF image selected. Use IDE/CF boot mode to launch it; it is not mounted to floppy drives.")
            return
        if not self.is_floppy_image(disk):
            self.status_var.set("Double-click mounting only supports floppy disk images.")
            return

        for drive in ("A", "B", "C", "D"):
            if not self.mounted_drive_vars[drive].get().strip():
        ...
        messagebox.showinfo(APP_NAME, "All floppy drives A-D are already mounted. Clear a drive first.")
        self.status_var.set("All floppy drives A-D are already mounted.")
    ...
    def reset_launch_reminders(self) -> None:
        proceed = messagebox.askyesno(
            APP_NAME,
            "Reset printer and tape launch reminders?\n\n"
            "The launcher will show the printer and tape help messages again when those features are used.",
        )
        if not proceed:
            return
        self.show_printer_launch_reminder_var.set(True)
        self.show_tape_launch_reminder_var.set(True)
        self.save_config()
        if hasattr(self, "diagnostics_text"):
            self.refresh_diagnostics()
        self.status_var.set("Launch reminders reset.")

    def show_launch_reminder_dialog(
        self,
        title: str,
        message: str,
        setting_var: tk.BooleanVar,
        dont_show_text: str,
    ) -> bool:
        """Show an OK/Cancel launch reminder with a persistent opt-out checkbox."""
        if not setting_var.get():
            return True
        ...
        ttk.Button(button_row, text="OK", command=on_ok).pack(side=tk.RIGHT, padx=(0, 8))
    ...
    def show_keyboard_help(self) -> None:
        help_text = (
            "Microbee / uBee512 key mapping for macOS\n"
            "========================================\n\n"
            "Quick MacBook equivalents\n"
            "-------------------------\n"
            "Most MacBook keyboards do not have dedicated Page Up, Page Down, Home, End,\n"
            "Insert, or Pause/Break keys. Use these Mac equivalents when uBee512 refers\n"
            "to PC-style keys:\n\n"
            "  Page Up              = Fn + Up Arrow\n"
            ...
            "  Ctrl                 = Control\n"
            "  Enter                = Return\n"
            "  Pause / Break        = usually unavailable on MacBook keyboards\n"
            "  Insert               = usually unavailable on MacBook keyboards\n\n"
            "Microbee key differences\n"
            "------------------------\n"
            "256TC / Teleterm models\n"
            "  SELECT               = Page Up\n"
            "                       = Fn + Up Arrow on MacBook\n\n"
            "Standard / Premium models\n"
            "  LINEFEED             = Page Up\n"
            "                       = Fn + Up Arrow on MacBook\n\n"
            "  RESET                = Page Down\n"
            "                       = Fn + Down Arrow on MacBook\n\n"
            "  SHIFT 0              = Shift + Insert\n"
            "                       = awkward on MacBook because Insert is not normally present\n\n"
            "All models\n"
            "  BREAK                = Pause / Break\n"
            "                       = may need an external keyboard or remapping on MacBook\n\n"
            "  Function keys        = Control + number\n"
            "                       = for example Control + 1, Control + 2, etc.\n\n"
            "Emulator control\n"
            "----------------\n"
            "  Exit emulator        = End\n"
            "                       = Fn + Right Arrow on MacBook\n\n"
            "  Reset emulator       = Page Down\n"
            "                       = Fn + Down Arrow on MacBook\n\n"
            "  Reset without prompt = Esc + Page Down\n"
            "                       = Esc + Fn + Down Arrow on MacBook\n\n"
            "EMUKEY\n"
            "------\n"
            "uBee512 uses an EMUKEY for emulator shortcuts.\n\n"
            "  EMUKEY               = Home or Alt\n"
            "                       = Fn + Left Arrow or Option on MacBook\n\n"
            "On 256TC / Teleterm models, Alt/Option is only usable if --lpen is enabled.\n"
            "If one EMUKEY option does not respond, try the other.\n\n"
            "Useful EMUKEY shortcuts on MacBook\n"
            "----------------------------------\n"
            "  Full screen          = EMUKEY + Return\n"
            "                       = Option + Return, or Fn + Left Arrow + Return\n\n"
            "  Tape rewind          = EMUKEY + T\n"
            "  Sound mute           = EMUKEY + S\n"
            "  Pause emulator       = EMUKEY + P\n"
            "  Volume up / down     = EMUKEY + Up Arrow / Down Arrow\n"
            "  Mouse wheel mode     = EMUKEY + W\n"
            "  Microbee mouse toggle= EMUKEY + M\n"
            "  Console mode         = EMUKEY + C\n"
            "  OpenGL filter toggle = EMUKEY + F\n\n"
            "  Power cycle          = EMUKEY + Page Down\n"
            "                       = Option + Fn + Down Arrow, or Fn + Left Arrow + Fn + Down Arrow\n\n"
            "Practical Mac notes\n"
            "-------------------\n"
            "  - The Mac Function key shortcuts depend on your keyboard settings.\n"
            "  - If function keys or navigation keys behave strangely, try holding Fn.\n"
            "  - For heavy emulator use, an external keyboard with Page Up, Page Down,\n"
            "    Home, End, Insert, and Pause/Break keys may be easier.\n"
            "  - If a key is important and not available, consider using macOS keyboard\n"
            "    remapping or a tool such as Karabiner-Elements.\n\n"
            "Printer reminder\n"
            "----------------\n"
        ...
        window.title("Keyboard Help - macOS")
    ...
    def launch_ubee(self) -> None:
        ...
        if self.advanced_mode_var.get() and printer_mode != "Off":
            ...
            proceed = self.show_launch_reminder_dialog(
                "Printer launch reminder",
                reminder,
                self.show_printer_launch_reminder_var,
                "Don't show this printer reminder again",
            )
            if not proceed:
        ...
        if self.normalize_boot_mode(self.boot_mode_var.get().strip()) != "Custom arguments only" and tape_text:
            ...
            proceed = self.show_launch_reminder_dialog(
                "Tape launch reminder",
                reminder,
                self.show_tape_launch_reminder_var,
                "Don't show this tape reminder again",
            )
            if not proceed:
                self.status_var.set("Launch cancelled.")
                return
        ...
        if self.advanced_mode_var.get() and library_path:
            ...
            env["DYLD_LIBRARY_PATH"] = f"{library_path}:{existing}" if existing else library_path
