#!/usr/bin/env python3
"""
Ubee512 Launcher for macOS

A macOS desktop launcher for Ubee512 that:
- uses tested macOS defaults
...
APP_NAME = "Ubee512 Launcher - macOS"

# This edition is intentionally macOS-specific.
IS_MAC = True

CONFIG_DIR = Path.home() / "Library" / "Application Support" / "Ubee512 Launcher"
CONFIG_FILE = CONFIG_DIR / "config.json"

DEFAULT_UBEE_EXECUTABLE = Path("/Applications/ubee512-6.0.0-macOS-arm64/ubee512/app/ubee512")
DEFAULT_LIBRARY_PATH = Path("/Applications/ubee512-6.0.0-macOS-arm64/ubee512/libs")
DEFAULT_SEARCH_ROOT = Path.home() / ".ubee512"
DEFAULT_ROM1_PATH = DEFAULT_SEARCH_ROOT / "roms" / "rom1.bin"
DEFAULT_BOOT_DISK = DEFAULT_SEARCH_ROOT / "disks" / "BOOT.DS80_"
DEFAULT_LOGO_PATH = DEFAULT_SEARCH_ROOT / "images" / "ubee512-logo.bmp"
...
DEFAULT_CONFIG = {
    ...
    "last_disk": str(DEFAULT_BOOT_DISK) if DEFAULT_BOOT_DISK.exists() else "",
...
class UbeeLauncherApp:
    ...
    def _build_ui(self) -> None:
        """Build a compact wide layout that fits better on MacBook displays."""
        self.root.columnconfigure(0, weight=1)
        self.root.rowconfigure(0, weight=0)
        self.root.rowconfigure(1, weight=1)
        self.root.rowconfigure(2, weight=0)

        # A wide Mac-style default. The user can still resize smaller.
        ...
        self.model_label = ttk.Label(controls, text="Model")
        ...
        ttk.Button(launch_panel, text="Reveal Disks", command=self.reveal_disks_folder).grid(row=3, column=2, sticky="w", pady=(6, 0), padx=(8, 0))
    ...
    def apply_mac_defaults(self) -> None:
        """Restore the tested macOS paths and perform a scan if possible."""
        self.ubee_var.set(str(DEFAULT_UBEE_EXECUTABLE))
        self.library_path_var.set(str(DEFAULT_LIBRARY_PATH))
        self.search_root_var.set(str(DEFAULT_SEARCH_ROOT))
        self.default_rom1_var.set(str(DEFAULT_ROM1_PATH))
        self.printer_output_var.set(str(DEFAULT_PRINTER_FILE))
        self.boot_mode_var.set("Plain launch")
        self.include_hidden_var.set(False)
        self.status_var.set("Mac defaults applied.")
        if DEFAULT_SEARCH_ROOT.exists():
            self.scan_files()
            self.select_disk_by_path(DEFAULT_BOOT_DISK)
        self.refresh_command_preview()

    def select_disk_by_path(self, disk_path: Path) -> None:
        """Select a disk in the listbox when it is present in the current scan results."""
        target = disk_path.expanduser()
        for i, path in enumerate(self.scan_results.disks):
            if path == target:
    ...
    def check_mac_setup(self) -> None:
        """Check the common files/folders needed by this macOS launcher."""
        checks = [
            ("uBee512 executable", DEFAULT_UBEE_EXECUTABLE, DEFAULT_UBEE_EXECUTABLE.is_file()),
            ("uBee512 libraries folder", DEFAULT_LIBRARY_PATH, DEFAULT_LIBRARY_PATH.is_dir()),
            ("User data folder", DEFAULT_SEARCH_ROOT, DEFAULT_SEARCH_ROOT.is_dir()),
            ("ROM1 file", DEFAULT_ROM1_PATH, DEFAULT_ROM1_PATH.is_file()),
            ("Disk images folder", DEFAULT_SEARCH_ROOT / "disks", (DEFAULT_SEARCH_ROOT / "disks").is_dir()),
            ("Default boot disk", DEFAULT_BOOT_DISK, DEFAULT_BOOT_DISK.is_file()),
            ("Logo image", DEFAULT_LOGO_PATH, DEFAULT_LOGO_PATH.is_file()),
        ]
        lines = []
        missing = []
        ...
        message = "macOS uBee512 setup check\n\n" + "\n\n".join(lines)
        if missing:
            messagebox.showwarning(APP_NAME, message)
            self.status_var.set("Setup check found missing items.")
        else:
            messagebox.showinfo(APP_NAME, message)
            self.status_var.set("Setup check passed.")

    def reveal_user_data_folder(self) -> None:
        self.open_path_in_file_manager(DEFAULT_SEARCH_ROOT)

    def reveal_disks_folder(self) -> None:
        self.open_path_in_file_manager(DEFAULT_SEARCH_ROOT / "disks")
