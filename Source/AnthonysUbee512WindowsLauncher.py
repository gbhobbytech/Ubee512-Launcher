...
DEFAULT_UBEE_EXECUTABLE = Path(r"C:\Program Files\ubee512\ubee512.exe")
DEFAULT_SEARCH_ROOT = Path(r"C:\Program Files\ubee512")
...
def default_cpmtools_path(name: str) -> str:
    root = DEFAULT_SEARCH_ROOT
    candidate_names = [name]
    if os.name == "nt" and not name.lower().endswith(".exe"):
        candidate_names.insert(0, f"{name}.exe")
    search_dirs = [root / "tools" / "cpmtools-2.10", root / "tools", root]
    for folder in search_dirs:
        for candidate_name in candidate_names:
            bundled = folder / candidate_name
            if bundled.exists():
                return str(bundled)
...
class UbeeLauncherApp:
    ...
    def auto_setup(self) -> None:
        ...
        if not DEFAULT_SEARCH_ROOT.exists():
            messagebox.showwarning(
                APP_NAME,
                "The uBee512 data folder was not found.\n\nInstall uBee512, run ubee512.exe once to create its folders and config files, then run Auto setup again.",
            )
            self.status_var.set("Auto setup needs the uBee512 install folder. Run ubee512.exe once first.")
    ...
    def refresh_diagnostics(self) -> None:
        ...
        libdsk_path = (root / "share" / "libdskrc") if os.name == "nt" else (Path.home() / ".libdskrc")
        lines.append(self.diagnostics_line("LibDsk config", libdsk_path.exists(), str(libdsk_path)))
        if os.name == "nt" and "program files" in str(root).lower():
            lines.append(self.info_line("Windows first-run note", "if uBee512 has not created ubee512rc, roms.alias, disks.alias or share\\libdskrc, run uBee512 once as Administrator"))
        library_path = self.library_path_var.get().strip()
        if library_path:
        ...
        if ubee_found:
            if os.name == "nt":
                lines.append(self.info_line("uBee512 command probe", "skipped on Windows to avoid opening emulator console windows"))
                lines.append(self.info_line("Model list probe", "skipped on Windows; using the launcher's supported model list"))
            else:
                try:
            ...
            if os.name != "nt" and ubee_path and ubee_path.exists():
                try:
    ...
    def launch_ubee(self) -> None:
        ...
        library_path = self.library_path_var.get().strip()
        if os.name != "nt" and self.advanced_mode_var.get() and library_path:
            existing = env.get("LD_LIBRARY_PATH", "")
            env["LD_LIBRARY_PATH"] = f"{library_path}:{existing}" if existing else library_path

        executable_path = Path(executable).expanduser()
        if executable_path.exists():
            launch_cwd = str(executable_path.parent)
        else:
            launch_cwd = str(self.get_search_root_path() if os.name == "nt" else Path.home())

        try:
            if os.name == "nt":
                subprocess.Popen(cmd, env=env, cwd=launch_cwd)
            else:
                subprocess.Popen(cmd, start_new_session=True, env=env, cwd=launch_cwd)
            self.save_config()
