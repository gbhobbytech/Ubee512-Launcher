...
class UbeeLauncherApp:
    ...
    def _build_ui(self) -> None:
        ...
        self.library_path_label = ttk.Label(top, text="Library path")
        self.library_path_label.grid(row=1, column=3, sticky="w", padx=(0, 6), pady=2)
        self.library_path_entry = ttk.Entry(top, textvariable=self.library_path_var)
        self.library_path_entry.grid(row=1, column=4, sticky="ew", pady=2)
        self.library_path_button = ttk.Button(top, text="Browse", command=self.choose_library_path, width=10)
        self.library_path_button.grid(row=1, column=5, padx=(6, 0), pady=2)
        ...
        self.clear_config_button = ttk.Button(controls, text="Clear config", command=self.clear_config, width=11)
        self.clear_config_button.grid(row=0, column=10, sticky="w", padx=(8, 0))
        ttk.Label(controls, textvariable=self.status_var).grid(row=0, column=11, sticky="e", padx=(12, 0))
        ...
        files_area.columnconfigure(4, weight=1)
        ...
        self.quick_panel = ttk.LabelFrame(files_area, text="Quick setup", padding=8)
        self.quick_panel.grid(row=0, column=4, rowspan=4, sticky="nsew", padx=(8, 0))
        self.quick_panel.columnconfigure(0, weight=1)
        self.quick_label = ttk.Label(self.quick_panel, textvariable=self.setup_info_var, justify=tk.LEFT, wraplength=260)
        self.quick_label.grid(row=0, column=0, sticky="nw")
    ...
    def update_mode_ui(self) -> None:
        ...
        # Simple mode keeps the launcher focused: choose uBee512, scan ~/.ubee512,
        # select/mount a disk image, and launch.  Advanced mode exposes ROMs,
        # tapes, printer capture, CP/M tools, library overrides, and custom args.
        advanced_widgets = [
            self.library_path_label,
            self.library_path_entry,
            self.library_path_button,
            self.model_label,
            self.model_entry,
        ...
        if advanced:
            ...
            self.files_area.columnconfigure(2, weight=1)
            self.files_area.columnconfigure(4, weight=0)
        else:
            self.tape_list.grid(row=1, column=2, rowspan=3, sticky="nsew", pady=(4, 0), padx=(0, 4))
            self.tape_scroll.grid(row=1, column=3, rowspan=3, sticky="ns", pady=(4, 0))
            self.quick_panel.grid(row=0, column=4, rowspan=4, sticky="nsew", padx=(8, 0))
            self.advanced_panel.grid_remove()
            self.middle.columnconfigure(1, weight=0)
            self.files_area.columnconfigure(2, weight=1)
            self.files_area.columnconfigure(4, weight=1)

        tab_ids = self.workflow.tabs()
        printer_visible = str(self.printer_tab) in tab_ids
        cpm_visible = str(self.cpm_tab) in tab_ids

        if advanced and not printer_visible:
            self.workflow.add(self.printer_tab, text="Printer / LPRINT")
        elif not advanced and printer_visible:
            self.workflow.forget(self.printer_tab)
    ...
    def update_setup_info(self) -> None:
        ...
        rom_dir = root / "roms"
        disk_dir = root / "disks"
        rom_count = len(self.scan_results.roms) if self.scan_results.roms else len(list(rom_dir.glob("*"))) if rom_dir.exists() else 0
        disk_count = len(self.scan_results.disks) if self.scan_results.disks else len(list(disk_dir.glob("*"))) if disk_dir.exists() else 0
        ...
        self.setup_info_var.set(
            ...
            "Tip: ROM override, CP/M tools, printer capture, model selection and many other options are available in Advanced mode."
        )
    ...
    def auto_setup(self) -> None:
        ...
        self.scan_files()
        self.select_first_boot_disk()
        self.update_setup_info()
        self.refresh_command_preview()
        self.status_var.set("Auto setup complete.")

    def select_first_boot_disk(self) -> None:
        preferred = {"boot.ds80_", "boot.dsk"}
    ...
    def build_command(self) -> list[str]:
        ...
        if self.advanced_mode_var.get() and model:
