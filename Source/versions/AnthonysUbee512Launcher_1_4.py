...
class UbeeLauncherApp:
    ...
    def diagnostics_line(self, label: str, ok: bool, detail: str = "") -> str:
    ...
    def refresh_diagnostics(self) -> None:
        ...
        lines.append(self.diagnostics_line("disks.alias", (root / "disks.alias").is_file(), str(root / "disks.alias")))
        ...
        if ubee_found:
            try:
                ...
                first_line = (result.stdout or result.stderr).splitlines()[0] if (result.stdout or result.stderr).splitlines() else "no output"
                lines.append(self.diagnostics_line("uBee512 responds", result.returncode in (0, 1), first_line))
    ...
    def build_command(self) -> list[str]:
        ...
        # Selecting a disk in the media list is only a selection. It must not
        # change the launch command in either Simple or Advanced mode. Disk
        # flags are added only when the user explicitly mounts a disk to A/B/C/D.
        if mounted_floppies:
    ...
    def launch_ubee(self) -> None:
        ...
        # No validation is needed for the selected media-list disk here because
